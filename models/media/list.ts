...
const appendQueryPointer = (last?: firebase.firestore.DocumentSnapshot) => (
  ...
  pipe(
    O.fromNullable(last),
    O.fold(() => query, () => query.startAfter(last)),
  );
...
const makeFetch = (limit: number, last?: firebase.firestore.DocumentSnapshot) =>
...
const fetch = (
  ...
  pipe(
    last,
    O.fold(() => makeFetch(limit), lastRef => makeFetch(limit, lastRef)),
  );
